...
pub async fn copy_chunk(
    ...
    let uncompressed_row_count = get_uncompressed_row_count(source_tx, &task.source_chunk).await?;
    let target_chunk_is_partial = chunk_status_is_partial(target_tx, &target_chunk).await?;

    let uncompressed_result = if uncompressed_row_count > 0
...
async fn get_uncompressed_row_count(
    source_tx: &Transaction<'_>,
    chunk: &SourceChunk,
) -> Result<i64> {
    let row = source_tx
